...
# rtf <- function(formula, subset) {
#   list(formula = formula, subset = substitute(subset))
# }

# 1 (or factor level 1) = lower bound; 2 = upper bound

create_data <- function(alpha, tau, beta, delta, drift_crit, 
                        data, rt, response, boundary, id,
                        pos_intercepts) {
  ...
  mmatrix_drift_crit <- split(as.data.frame(model.matrix(drift_crit, data = data_ordered)), f = data_ordered[,response])
  ...
  n_par <- vapply(list(mmatrix_alpha, mmatrix_tau, mmatrix_beta, mmatrix_delta, mmatrix_drift_crit), function(x) ncol(x[[1]]), 1)
  ...
  list(N=N,
       ...
       mmatrix_drift_crit_u = as.matrix(mmatrix_drift_crit[[2]]),
       mmatrix_drift_crit_l = as.matrix(mmatrix_drift_crit[[1]]),
       boundary_u = (as.numeric(factor(d_split[[2]][,boundary]))-1.5)*2,
       boundary_l = (as.numeric(factor(d_split[[1]][,boundary]))-1.5)*2,
       n_par = n_par,
...
wiener_drift_init <- function(data) {
  N <- data$N
  ...
  tmp_out$tau[data$pos_intercept[2,seq_len(data$n_intercept[2])]] <- runif(data$n_intercept[2], 0.2, 0.25)
  ...
  tmp_out$drift_crit <- rep(0, data$n_par[5])
  tmp_out$drift_crit[data$pos_intercept[5,seq_len(data$n_intercept[5])]] <- rnorm(data$n_intercept[5], 0, 1)
  tmp_out$drift_crit[data$pos_other_pars[5,seq_len(data$n_other_pars[5])]] <- rnorm(data$n_other_pars[5], 0, 0.1)
